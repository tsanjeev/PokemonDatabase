...
if(!$mysqli || $mysqli->connect_errno){
  echo "Connection error ". $mysqli->connect_errno . " " . $mysqli->connect_error;
    ...
    if(!$stmt = $mysqli2->prepare('SELECT loc_id FROM location WHERE loc_id = ?')){
    		echo "Prepare 2 failed: "  . $stmt->errno . " " . $stmt->error;
    	};
    if(!$stmt->bind_param('s', $_POST['pokeLoc'])){
    ...
    $mysqli3 = new mysqli("oniddb.cws.oregonstate.edu","sanjeevt-db","x6wwQ7Pqw8iQg7b8", "sanjeevt-db");
        if(!$stmt = $mysqli3->prepare('INSERT INTO poke_loc (lid, pid) VALUES (?, ?)')){
        	echo "Prepare 3 failed: "  . $stmt->errno . " " . $stmt->error;
        };
        if(!$stmt->bind_param('ii', $lid, $pid)){
