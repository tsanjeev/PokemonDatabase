...
if(!$mysqli || $mysqli->connect_errno){
  echo "Connection error ". $mysqli->connect_errno . " " . $mysqli->connect_error;
    ...
    if(!$stmt = $mysqli2->prepare('SELECT skill_id FROM skill WHERE skill_id = ?')){
    		echo "Prepare 2 failed: "  . $stmt->errno . " " . $stmt->error;
    	};
    if(!$stmt->bind_param('s', $_POST['pokeSkill'])){
    ...
    $mysqli3 = new mysqli("oniddb.cws.oregonstate.edu","sanjeevt-db","x6wwQ7Pqw8iQg7b8", "sanjeevt-db");
        if(!$stmt = $mysqli3->prepare('INSERT INTO poke_skill (sid, pid) VALUES (?, ?)')){
